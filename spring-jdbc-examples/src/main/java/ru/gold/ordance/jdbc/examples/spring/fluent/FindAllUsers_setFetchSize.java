...
public class FindAllUsers_setFetchSize {

    private static final Logger LOGGER = LoggerFactory.getLogger(FindAllUsers_setFetchSize.class);
    ...
    private static List<User> findAllUser() {
        return jdbc.sql(QUERY).withFetchSize(FETCH_SIZE).query((rs, num) -> MAPPER.map(rs)).list();
    }
}
